...
<li>
<a class="${currentPage.includes('createurs.html') ? 'active' : ''}" href="${base}pages/createurs.html">
<i class="fa-solid fa-users"></i><span>Créateurs</span>
</a>
</li>
<li>
...
<li>
<a class="${(currentPage.includes('discussion.html') || currentPage.includes('chat.html') || currentPage.includes('forum.html')) ? 'active' : ''}" href="${base}pages/discussion.html">
<i class="fa-solid fa-comments"></i><span>Discussion</span>
</a>
</li>
<li>
...
// Ordre : Bienvenue -> Quiz -> Concours -> Culture -> Créateurs -> Illustration -> Bibliothèque -> Événements -> News -> Chat -> Forum
const heroContent = [
{ title: "Bienvenue sur QuizTable", button: "Voir le guide", link: "pages/guide.html" },
...
{ title: "Découvrez nos créateurs", button: "Voir les créations", link: "pages/createurs.html" },
...
const suggestionConfirmation = document.getElementById("suggestionConfirmation");
const suggestionPseudo = document.getElementById("suggestionPseudo");
const suggestionPseudoField = suggestionPseudo ? suggestionPseudo.closest(".suggestion-field") : null;
const suggestionPseudoError = document.getElementById("suggestionPseudoError");
const identityRadios = document.querySelectorAll('input[name="identity"]');

if (suggestionToggle && suggestionForm) {
suggestionToggle.addEventListener("click", () => {
suggestionForm.classList.toggle("active");
if (suggestionConfirmation) {
suggestionConfirmation.classList.remove("active");
}
});
}

function isAnonymeSelected() {
const checked = document.querySelector('input[name="identity"]:checked');
return checked ? checked.value === "anonyme" : false;
}

identityRadios.forEach(radio => {
radio.addEventListener("change", () => {
const anonyme = isAnonymeSelected();
if (suggestionPseudoField) {
suggestionPseudoField.style.display = anonyme ? "none" : "flex";
}
if (suggestionPseudo) {
suggestionPseudo.required = !anonyme;
if (anonyme) {
suggestionPseudo.value = "";
suggestionPseudo.classList.remove("error");
if (suggestionPseudoError) suggestionPseudoError.classList.remove("active");
}
}
});
});
...
if (!isAnonymeSelected() && suggestionPseudo && suggestionPseudo.value.trim() === "") {
suggestionPseudo.classList.add("error");
if (suggestionPseudoError) suggestionPseudoError.classList.add("active");
suggestionPseudo.focus();
return;
}

suggestionForm.reset();
suggestionForm.classList.remove("active");
if (suggestionPseudo) suggestionPseudo.classList.remove("error");
if (suggestionPseudoError) suggestionPseudoError.classList.remove("active");
if (suggestionPseudoField) suggestionPseudoField.style.display = "flex";
if (suggestionConfirmation) {
suggestionConfirmation.classList.add("active");
}
});
}
